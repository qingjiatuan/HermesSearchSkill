...
Layer 2: AnySearch (实时搜索 + 15+垂直领域) ✅ 免费限额
    ↓ 结果<8条 或 平均分<0.7
Layer 3: Agent Reach (平台定向搜索) ✅ 完整实现（安装后自动启用）
    ↓ 结果<12条 + 已安装插件
Layer 4: Firecrawl (深度全文抓取) ✅ 完整实现（需显式传入 enable_firecrawl=True）
    ↓
结果融合层 → 去重 + 评分 + 排序 + 质量报告
```
...
**A**: 建议完整安装以体验全部四层能力，但 Layer 3/4 有额外使用门槛，可按需选择：

| 层级 | 安装要求 | 启用方式 | 说明 |
|------|---------|---------|------|
| **Layer 1-2** | 必须安装 | 自动启用 | 通用搜索能力，完全免费，覆盖 80% 场景 |
| **Layer 3 (Agent Reach)** | 建议安装 | 安装后自动启用 | 社交媒体/垂直平台定向搜索，部分平台需配置 Cookie |
| **Layer 4 (Firecrawl)** | 按需安装 | 显式传入 `enable_firecrawl=True` | 深度全文抓取，有 API 调用成本，默认不自动启用避免意外扣费 |

> 💡 **设计考量**：Firecrawl 默认不自动启用是为了避免用户在不知情的情况下产生 API 费用。如果需要深度抓取能力，建议先配置好 API Key，然后在调用时显式启用。
